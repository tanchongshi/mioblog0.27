			...
			for (var i = 0, len = fileList.length; i < len; i++) {
				...
				if(data.is_dir == false){
					(function(data) { 
						...
						//修改文件
						K(row[0].insertCell(2)).addClass('tdEditor').html("修改文件").click(function(event) {
...
div.mouseover(function(e) {//滑过这个div的时候
	...
	data.is_photo&&K(this).children().eq(3).css("display","block");//如果是图片格式就查找当前下面的第4个标签元素设置为可见
})
.mouseout(function(e) {//划出这个循环div时
	...
	data.is_photo&&K(this).children().eq(3).css("display","none");//如果是图片格式设置当前下第4个标签为隐藏
});
				div.append(photoDiv);
...
if(data.is_photo){//如果是图片
	...
	var _span2 = K('<span class="xl_span2" data-url="'+K.formatUrl(result.current_url + data.filename, 'absolute')+'" style="position:absolute;display:none;width:102px;background:#0610d2;color:#FFF;text-align:center; cursor: pointer;line-height:20px;bottom:-23px;left:0;">引图</span>');
    div.append(_span);
	div.append(_span2);
}				
			}
